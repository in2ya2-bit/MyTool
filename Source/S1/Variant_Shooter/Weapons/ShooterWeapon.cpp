...
void AShooterWeapon::BeginPlay()
{
	...
	// apply weapon data asset overrides if assigned
	if (WeaponData)
	{
		FiringRecoil = WeaponData->FiringRecoil;
		bFullAuto = WeaponData->bFullAuto;
		RefireRate = WeaponData->RefireRate;
		MagazineSize = WeaponData->MagazineSize;
		AimVariance = WeaponData->AimVariance;
		ProjectileSpeedOverride = WeaponData->ProjectileSpeed;
		FireCameraShakeClass = WeaponData->FireCameraShakeClass;
		CameraShakeScale = WeaponData->CameraShakeScale;
...
void AShooterWeapon::FireProjectile(const FVector& TargetLocation)
{
	...
	// override projectile speed if configured
	if (Projectile && ProjectileSpeedOverride > 0.0f)
	{
		if (UProjectileMovementComponent* ProjMovement = Projectile->FindComponentByClass<UProjectileMovementComponent>())
		{
			ProjMovement->InitialSpeed = ProjectileSpeedOverride;
			ProjMovement->MaxSpeed = ProjectileSpeedOverride;
			ProjMovement->Velocity = Projectile->GetActorForwardVector() * ProjectileSpeedOverride;
		}
	}
	...
	if (HorizontalRecoil > 0.0f)
	{
		if (APawn* OwnerPawn = Cast<APawn>(GetOwner()))
		{
			OwnerPawn->AddControllerYawInput(FMath::FRandRange(-HorizontalRecoil, HorizontalRecoil));
		}
	}
...
{
	// use the visible mesh's muzzle socket (FP when in first person, TP when in third person)
	USkeletalMeshComponent* ActiveMesh = FirstPersonMesh->IsVisible() ? FirstPersonMesh : ThirdPersonMesh;
	const FVector MuzzleLoc = ActiveMesh->GetSocketLocation(MuzzleSocketName);
